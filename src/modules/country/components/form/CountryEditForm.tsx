...
import { CountryDataRow, CountryUpdateFormValues } from "../../types";
import { Button, Flex, Radio, Stack, Text, TextInput } from "@mantine/core";
import { FC } from "react";
import { useUpdateCountry } from "../../hooks/useUpdateCountry";

type CountryEditFormProps = {
  country: CountryDataRow;
  id: string;
  close: () => void;
};

export const CountryEditForm: FC<CountryEditFormProps> = ({
  country,
  id,
  close,
}) => {
  const { mutate, isPending } = useUpdateCountry();
  const form = useForm({
  ...
  const onSubmit = async (values: CountryUpdateFormValues) => {
...
CountryEditForm.displayName = "CountryEditForm";
